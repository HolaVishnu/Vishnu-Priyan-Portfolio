...
export default function Hud() {
  ...
  useEffect(() => {
    return useUniverse.subscribe((s) => {
      ...
      if (posRef.current) {
        posRef.current.textContent = `X ${fmt(tmpPos.x)}  Y ${fmt(
          tmpPos.y,
        )}  Z ${fmt(tmpPos.z)}`;
      }
      if (velRef.current) {
        velRef.current.textContent = `${Math.min(
          Math.abs(s.velocity) * 0.42,
          99,
        ).toFixed(1)} PC/S`;
      }
      ...
      {phase === "journey" && !focusedProject && projectFlightPhase === "idle" && (
        ...
        >
          ...
          <div className="absolute right-6 top-6 flex flex-col items-end gap-2 text-right md:right-10 md:top-8">
            ...
            {audioOn && nowPlaying && (
              <p className="max-w-[18rem] truncate font-mono text-[9px] uppercase tracking-[0.25em] text-cyan/80 md:max-w-[22rem]">
                NOW {nowPlaying}
              </p>
            )}
            <a
            ...
            >
              <span
                ...
                style={{
                  boxShadow: "0 0 5px rgba(200,107,255,0.8)",
          ...
          <div className="hud-telemetry absolute bottom-6 left-6 hidden font-mono text-[10px] uppercase tracking-[0.25em] md:block md:bottom-8 md:left-10">
          ...
          <p
            ref={hintRef}
            className="hud-scroll-hint absolute bottom-8 right-6 hidden font-mono text-[10px] uppercase tracking-[0.35em] text-dim transition-opacity duration-700 md:block md:right-24"
          >
            Scroll to travel ↓
          </p>
